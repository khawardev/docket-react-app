...
const NewFolder = (props: folderprops) => {
    ...
    console.log("🚀 ~ file: FolderInterface.tsx:27 ~ NewFolder ~ folderData:", folderData)
    const lastIndex = folderData.length - 1;
    // console.log("🚀 ~ file: FolderInterface.tsx:40 ~ NewFolder ~ lastIndex:", lastIndex)
    return (
        <>
            <div className=' select-none'>
                ...
                <section className='w-11/12 m-auto mt-7 grid md:grid-cols-5 sm:grid-cols-3 grid-cols-2 sm:gap-4 gap-2 '>
                    {folderData.map((index) => (
                        <main key={index} className=' bg-slate-100 sm:p-5 p-4 rounded-2xl hover:cursor-pointer hover:bg-slate-200'>
                            <div className=' flex items-center  justify-between  '>
                                <p className=' bg-purple-400 rounded-full sm:px-3 px-2 text-white sm:text-base text-sm'>{folderData[lastIndex]}</p>
